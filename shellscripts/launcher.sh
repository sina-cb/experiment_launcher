...
while [[ -z "$gazeboProc" ]]; do 
	...
	xterm -e "roslaunch multi_robot_world.launch" &
	gazeboXterm=$!
...
while [[ -z "$cmvisionProc" ]]; do 
	...
	xterm -e "roslaunch cmvision_node_multi.launch" &
	cmvisionXterm=$!
...
while :; do 
	...
	xterm -e "roslaunch amcl_multiple_robots.launch" &
	amclXterm=$!
...
while [[ -z "$rvizProc" ]]; do 
	...
	xterm -e "roslaunch multi_nested_rviz.launch" &
	rvizXterm=$!
...
while [[ -z "$behaveProc" ]]; do 
	...
	xterm -e "roslaunch multi_robot_behaviour.launch" &
	behaveXterm=$!
...
# Script for guaranteed safe start of floor1_robot2
floor1_robot2Xterm=
floor1_robot2Proc=

while [[ -z "$floor1_robot2Proc" ]]; do 
	
	xterm -e "rosrun experiment_runner floor1_robot2" &
	floor1_robot2Xterm=$!
	# echo "floor1_robot2Xterm="$floor1_robot2Xterm

	sleep 2s

	floor1_robot2Proc=$(pidof floor1_robot2)
	# echo "floor1_robot2Proc="$floor1_robot2Proc

	if test -z "$floor1_robot2Proc"; then
		kill $floor1_robot2Xterm
		sleep 5s
		kill -9 $floor1_robot2Xterm
		sleep 5s
		echo "Had to force kill floor1_robot2 xterm...restarting it"
	fi
done



# Script for guaranteed safe start of floor1_robot1
floor1_robot1Xterm=
floor1_robot1Proc=

while [[ -z "$floor1_robot1Proc" ]]; do 
	
	xterm -e "rosrun experiment_runner floor1_robot1" &
	floor1_robot1Xterm=$!
	# echo "floor1_robot1Xterm="$floor1_robot1Xterm

	sleep 2s

	floor1_robot1Proc=$(pidof floor1_robot1)
	# echo "floor1_robot1Proc="$floor1_robot1Proc

	if test -z "$floor1_robot1Proc"; then
		kill $floor1_robot1Xterm
		sleep 5s
		kill -9 $floor1_robot1Xterm
		sleep 5s
		echo "Had to force kill floor1_robot1 xterm...restarting it"
	fi
done
